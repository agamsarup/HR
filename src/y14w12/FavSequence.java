...
public class FavSequence {
	...
	static void solve() {
		...
		Map<Integer, HashSet<Integer>> edgeList = new HashMap<Integer, HashSet<Integer>>();
		...
		for (int i = 0; i < n; i++) {
			...
			for (int j = 0; j < k - 1; j++) {
				...
				if (!edgeList.containsKey(v1))
					edgeList.put(v1, new HashSet<Integer>());
...
//			sortedResult.add(curNode);
			...
			if (edgeList.containsKey(curNode)) {
				Set<Integer> EdgesToRemove = edgeList.get(curNode);
